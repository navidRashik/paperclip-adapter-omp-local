...
export function parseOmpJsonl(stdout: string): ParsedOmpOutput {
  ...
  for (const rawLine of stdout.split(/\r?\n/)) {
    ...
    // Session header: OMP emits `{ "type": "session", "id": "<uuid>", "cwd": ... }`
    // as the first line. Capture the id so the adapter can resume this exact
    // session on the next heartbeat with `--resume <id>`.
