...
function stringifyToolPayload(value: unknown): string {
  ...
  if (value && typeof value === "object" && Array.isArray((value as any).content)) {
    var inner = extractTextContent((value as any).content);
...
const SILENT_TYPES: Record<string, number> = {
  response: 1,
...
export function createOmpStdoutParser(): { parseLine: (line: string, ts: string) => TranscriptEntry[]; reset: () => void } {
  const pendingToolCalls = new Map<string, { toolName: string }>();
  ...
  function parseLine(line: string, ts: string): TranscriptEntry[] {
    ...
    if (type === "agent_end") {
      const endEntries: TranscriptEntry[] = [];
...
// Module-level parser so the existing stateless entry point keeps working.
const defaultParser = createOmpStdoutParser();
